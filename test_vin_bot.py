...
async def api_get_parts_by_vin(session, vin: str, cat: str) -> dict:
    cache_key = f"parts2:{vin}:{cat}"
    cached = cache_get(cache_key)
    if cached is not None and isinstance(cached, dict):
        print(f"[CACHE HIT] getPartsbyVIN {vin} cat={cat}")
        _log_partsapi_result(vin, cat, cached)
        return cached

    payload, http_status, _raw_text = await _request_parts_by_vin_raw(session, vin, cat, timeout=25)
    result = _classify_partsapi_payload(payload, http_status=http_status, cat=cat)

    # Кешируем только надежные ответы:
    # - OK
    # - NO (валидно пусто)
    # ERR не кешируем, чтобы не цементировать временные проблемы
    if result["ok"] or result["is_empty"]:
        cache_set(cache_key, result)

    _log_partsapi_result(vin, cat, result)
    return result
...
async def fetch_parts_with_cat_fallback(
    ...
    """
    Возвращает:
    - actual_cat: cat, на которой остановились
    - result: нормализованный dict из api_get_parts_by_vin()

    Приоритет:
    1. Первый OK
    2. Если OK нет, но был NO -> вернуть первый NO
    3. Если были только ERR -> вернуть первый ERR
    """
    cats_to_try = [cat] + ALTERNATIVE_CATS.get(cat, [])

    first_no_result = None
    first_no_cat = None

    first_err_result = None
    first_err_cat = None
    ...
    for try_cat in cats_to_try:
        ...
        if result["is_empty"] and first_no_result is None:
            first_no_result = result
            first_no_cat = try_cat

        if result["is_error"] and first_err_result is None:
            first_err_result = result
            first_err_cat = try_cat

    if first_no_result is not None:
        return first_no_cat, first_no_result

    if first_err_result is not None:
        return first_err_cat, first_err_result

    # Совсем защитный fallback
    return cat, _build_partsapi_result(
        ok=False,
        is_error=True,
        is_empty=False,
        status="UNKNOWN",
        http_status="UNKNOWN",
        message="Не удалось определить итоговый статус getPartsbyVIN",
        total=0,
        items=[],
        raw=None,
        cat=cat,
