...
TRUCK_BRANDS = {
    "MERITOR", "RENAULT TRUCKS", "VOLVO", "DAF", "SCANIA", "MAN", "IVECO",
...
# ══════════════════════════════════════════════════════════════════
#  Используются, когда:
#    • getPartsbyVIN не отвечает / возвращает пусто (виснет cat=7 и др.)
...
OEM_FALLBACK_ARTICLES: dict[str, dict[str, list[str]]] = {
    "TOYOTA": {
        ...
        "9":   ["23300-0Y060", "77024-30040"],
        "424": ["87139-50100", "87139-30040"],
        "281": ["04465-33471", "04465-02220", "04466-33130", "04465-33450"],
        "82":  ["43512-33130", "43512-06150", "42431-33130"],
        "5":   ["43460-09Y50", "43030-12010"],
        "13":  ["43430-06210", "43430-33060"],
        "1041":["48530-09Y00", "48510-09L80"],
        "686": ["90919-01233", "90919-01247"],
        "306": ["13568-09070", "13568-09071"],
        "305": ["90916-02633", "99367-K1230"],
        "1046":["16100-39436", "16100-29415"],
        "316": ["90916-03100", "90916-A3001"],
    },
    "LEXUS": {
    ...
    "HONDA": {
        ...
        "281": ["45022-S5A-E50", "43022-TA0-A00"],
        "82":  ["45251-S5A-N10"],
        "686": ["98079-5614P"],
    },
    "MITSUBISHI": {
...
async def api_get_parts_by_vin(session, vin: str, cat: str) -> list:
    # Короткий таймаут: partsapi для некоторых cat (cat=7 и др.) виснет без ответа.
    # 25 секунд — достаточно для нормальных ответов, но быстро отдаёт управление при зависании.
    data = await async_get(session, {
...
def _suffix_after_dash(article: str) -> str:
    """Хвост после последнего дефиса; если дефиса нет — весь нормализованный артикул."""
    a = article.strip()
    if "-" in a:
...
def _article_sort_key(
    ...
    """
    Ключ сортировки кандидатов на «основной» OEM. Меньше = лучше.
      1. Своя марка вперёд.
      2. Длиннее совпавший префикс из OEM_PREFIXES — точнее попадание.
      3. Цифровой суффикс после дефиса предпочтительнее буквенного
         (17801-28030 > 17801-0H030).
      4. Популярность: артикул, встречающийся у нескольких брендов (TOYOTA+LEXUS),
         предпочтительнее уникального.
      5. Короче нормализованный артикул.
      6. Лексикографически — для стабильности.
    """
    norm = normalize_article(article)
    suffix = _suffix_after_dash(article)
    is_own_brand = brand.upper() != manu_upper          # True = чужой = хуже
    ...
    return (
        is_own_brand,       # своя марка вперёд
        -prefix_len,        # длиннее совпавший префикс лучше
        suffix_has_letter,  # цифровой суффикс лучше (17801-28030 > 17801-0H030)
        -pop,               # популярнее лучше
        len(norm),          # короче лучше
        norm,               # стабильность
    )
...
def pick_primary_oem(
    ...
    # Популярность артикула по ВСЕМ брендам в ответе (до фильтрации по марке)
    popularity: dict[str, int] = {}
    for _, a in parts:
        key = normalize_article(a)
        popularity[key] = popularity.get(key, 0) + 1

    # Только наша марка (если есть)
...
async def try_oem_fallback(
    ...
    """
    Fallback-поиск, когда getPartsbyVIN не вернул полезных данных:
      • API завис / отдал пусто (например cat=7 на Toyota Camry ASV50);
      • все артикулы отфильтрованы (только грузовые бренды, например cat=281).

    Перебирает кандидаты из OEM_FALLBACK_ARTICLES[manu][cat] через getCrosses.
    Возвращает первый кандидат, у которого есть аналоги, или None.
    """
    manu_upper = manu_name.upper() if manu_name else ""
...
async def cmd_vin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args or len(context.args) < 2:
        await update.message.reply_text(
            ...
            "<code>/vin XW7BF4FK60S145161 масляный фильтр</code>\n"
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for cat_id, cat_label in cats_list:
            ...
            # Шаг 2: getPartsbyVIN (таймаут 25 с — быстро реагирует на зависание API)
            ...
            # ── FALLBACK: API пустой ИЛИ всё отсеяно фильтром грузовых ──────
            if api_empty or not all_parts:
                reason = (
                    "API не отдаёт данные для этого VIN/cat"
                    if api_empty else
                    "найденные артикулы отфильтрованы как грузовые"
                )
                fallback = await try_oem_fallback(session, manu_name, cat_id)
                if not fallback:
                    await update.message.reply_text(
                        f"❌ <b>{group_name}</b> — не удалось подобрать.\n"
                        f"<i>{reason}, типовой OEM для "
                        f"{manu_name or '?'} в категории {cat_id} не задан.</i>",
                        parse_mode="HTML",
                    )
                    continue
                ...
                msg = "\n".join([
                    ...
                    *([f"🚗 {car_str}"] if car_str else []),
                    ...
                    f"  ✅ <b>{fb_brand}</b>  <code>{fb_art}</code>",
                    "",
                    f"🔄 <b>Аналоги ({len(fb_crosses)}) — {fb_brand} <code>{fb_art}</code>:</b>",
                    cross_block,
                    "",
                    "⚠️ <i>Конкретная модификация не определена API — "
                    "сверь номер с каталогом перед заказом.</i>",
                ])
